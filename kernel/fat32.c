...
void readBootSector(drive_info hardDrive, uint32_t partitionAddr, FAT32_Metadata* infoFat){ //we get all the metadata
...
void updateEveryFat(FAT32_Metadata infoFat, int sectorFirstFat){ //we update every FAT table
    for (int i = 0; i < infoFat.nbFats; i++){ 
        writeDiskSector(infoFat.hardDrive, (uintptr_t) diskBuffer, sectorFirstFat + i * infoFat.sectorPerFat);
    }
}

int allocateFreeCluster(FAT32_Metadata infoFat, int lastCluster){ //we allocate a new cluster if it's needed
    ...
    while(indexSector < infoFat.fatLBA + infoFat.sectorPerFat){
        ...
        if (((uint32_t *)diskBuffer)[clusterIndex%128] == 0){            
            ((uint32_t *)diskBuffer)[clusterIndex%128] = 0x0FFFFFFF; //we make the new cluster an end cluster in the FAT table
            updateEveryFat(infoFat, indexSector); //we update the fat table
            for (int i = 0; i < 512; i++){ //we wipe the whole new cluster
                diskBuffer[i] = 0;
            }
            for (int i = 0; i < infoFat.sectorPerCluster; i++){ 
            ...
            if (lastCluster){ //if this is meant to continue a cluster chain we update the cluster before accordingly
...
void readFileName(unsigned char *fat32Name, unsigned char* filename){ //FAT32 has a specific format that we convert to a normal filename
...
void getToDirEl(FAT32_Metadata infoFat, uint32_t dirCluster, uint32_t index, uint8_t **record, int *sector){
...
void removeFATChain(FAT32_Metadata infoFat, uint32_t cluster){ //used to remove linked list in FAT table
